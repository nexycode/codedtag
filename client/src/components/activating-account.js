...
var ActivitingAccount = () => {
    ...
    const error = res => {
        if(res.response.data.is_error) {
            result.current.innerHTML = res.response.data.data;
            result.current.classList.add("error");
            result.current.classList.add("show");               
        }
    };
    ...
    // Verify user
    return (<div className='custom-field-form text-center wrapper max-960 offset-left offset-right ptb-50 hero  plr-15'>
        <p ref={result} className='highlighted-block response'>Currently it is verifying your account. Please wait a few moments, and you will be redirected to your account!</p>
    </div>);
